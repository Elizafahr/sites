...
var getDefValue = function getDefValue() {
    ...
    if (dropdownInput.value === 'credit-realty') {
        ...
        /* добавляет параметры для инпута(срок) */
        ...
        generalSum = defValue;    /* записывает defValue в переменную где зраниться общая сумма */


        inputRealty.min = autoMin;    /* добавляет min/max для инпута(первоначальный взнос) */

        inputRealty.max = autoMax;

        inputRealty.value = defValue;    /* добавляет defVal для инпута(первоначальный взнос) */


        inputFirstpay.value = generalSum * 20 / 100;    /* добавляет значение для инпута(первоначальный взнос) */
        ...
        generalSum = defValue;    /* записывает defValue в переменную где храниться общая сумма */
        ...
        inputRealty.value = defValue;    /* добавляет defVal для инпута(credit-realty) */
...
for (var i = 0; calcStepCheckbox.length > i; i++) {
    ...
    ckeckbox.addEventListener('click', function () {
        setDefProcent();    /* После ввода стоимости недвижимости, должно автоматически проставляться минимальное значение первоначального взноса. */


        calcSumCredit();    /* получает сумму кредита */


        getProcentRate();    /* получает % ставку */


        getSumYears();    /* получает количество лет */


        getPayMonth();    /* вычисляет ежемесячный платеж */


        getOffer();    /* показывает offer */
...
var getOffer = function getOffer() {
    if (dropdownInput.value === 'credit-realty') {
        ...
        if (sumCredit < 200000) {
            offerFailedText.textContent = "Наш банк не выдает автокредиты меньше 200000 рублей";
            offerFailed.classList.remove('visually-hidden');
            offerSuccess.classList.add('visually-hidden');
        } else {
            offerFailed.classList.add('visually-hidden');
...
inputRealty.oninput = function () {
    ...
    if (!checkGeneralSum(inputRealty)) {  /* проверяет сумму, если возвращает false то проверка не пройдена */
        return;
    }
...
calcPlus.onclick = function (e) {
    ...
    setCurProcent();  /* получает текущий % */
    calcSumCredit();  /* получает сумму кредита */
    getProcentRate();  /* получает % ставку */
    getSumYears();  /* получает количество лет */
    getPayMonth();  /* вычисляет ежемесячный платеж */
...
inputFirstpay.onchange = function () {
    ...
    setDefProcent();  /* получает текущий % */
    calcSumCredit();  /* получает сумму кредита */
    getProcentRate();  /* получает % ставку */
    getSumYears();  /* получает количество лет */
    getPayMonth();  /* вычисляет ежемесячный платеж */
...
var closeOverlay = function closeOverlay(target) {
    ...
    document.querySelector('#calc-step2').classList.add('visually-hidden')
