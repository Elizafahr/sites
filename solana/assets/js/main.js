...
navBtn.onclick = function () {
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    function animateStories() {
        ...
        const containerTop = storiesContainer.getBoundingClientRect().top + scrollPosition;
        const imgContainerTop = imgContainer.getBoundingClientRect().top + scrollPosition;
 ...
 // cursor
  ...
  document.addEventListener('mousemove', (e) => {
    ...
    cursor.style.backgroundColor = `rgb(${e.clientX % 25}, ${e.clientY % 255}, ${Math.abs(e.clientX - e.clientY) % 255})`;
});
...
const swiperRewiews = swiperElRewiews.swiper;
 ...
 function updateSlidesPerView() {
  if (window.innerWidth <= 600) { // Пример условие для мобильных устройств
    swiperRewiews.params.slidesPerView = 1;
  }
  else if (window.innerWidth <= 768) { // Пример условие для мобильных устройств
    swiperRewiews.params.slidesPerView = 2;
  } 
  else if (window.innerWidth <= 1368) { // Пример условие для мобильных устройств
